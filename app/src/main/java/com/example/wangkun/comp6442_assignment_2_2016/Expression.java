...
public abstract class Expression {
    ...
    public static Expression parse(String str, int signal){
        ...
        if (haveOperator(str, '+') || haveOperator(str, '-') || haveOperator(str, 'm')) {
            //if there is a negative or positive operator in a bracket, here need add a zero before the operator
            if (str.charAt(0) == '+' || str.charAt(0) == '-' || str.charAt(0) == 'm')
                str = "0" + str;
            ...
            for (char c : str.toCharArray()) {
                ...
                if (c == '+') {
                    ...
                    if (haveOperator(substr2, '+') || haveOperator(substr2, '-') || haveOperator(substr2, 'm')) {
                        continue;
                    }
                    ...
                    return new Min(parse(substr1, signal), parse(substr2, signal));
                }
            }
        }
